...
import { Exam } from '../models/Exam';  // ✅ استيراد مودل الامتحان
...
const studentAuth = (req: Request, res: Response, next: any) => {
...
router.get('/:id', studentAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const userEmail = req.headers['x-user-email'] as string;
    const userId = (req.headers['x-user-id'] as string) || ''; // اختياري إذا أرسل من العميل
    const { BlockedUser } = await import('../models/BlockedUser');
    if (userId) {
    ...
    if (!allowRetake) {
      const existing = await ExamResult.findOne({ examId: String(id), userEmail }).lean();
      if (existing) {
...
router.get('/', studentAuth, async (req, res) => {
  try {
    ...
    const sanitizedExams = exams.map((exam: any) => ({
      ...exam,
      questions: exam.questions.map((q: any) => ({
        id: q.id,
        type: q.type,
...
router.post('/submit', async (req, res) => {
  try {
    const {
      examId, userId, userEmail, userName, examTopic,
      answers, essayAnswers, duration, timeSpent,
      language = 'ar'
    } = req.body;
    ...
    const processedQuestions = (exam.questions || []).map((q: any) => {
      const correctIdx = Array.isArray(q.options)
        ? q.options.findIndex((opt: any) => opt?.isCorrect === true)
        : -1;
      return {
        id: String(q.id || ''),
        question: q.text?.[language] || q.text?.ar || q.text?.en || '',
        options: (q.options || []).map((opt: any) => 
          typeof opt === 'string' ? opt : (opt?.text?.[language] || opt?.text?.ar || opt?.text?.en || '')
        ),
...
router.post('/retake-request', studentAuth, async (req, res) => {
  try {
    ...
    const { RetakeRequest } = await import('../models/RetakeRequest');
    const doc = await RetakeRequest.findOneAndUpdate(
      { examId: String(examId), userId: String(userId) },
      { $set: { userEmail, status: 'pending', reason: reason || '' } },
