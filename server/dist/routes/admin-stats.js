        ...
        const processedExam = {
            ...
            questions: exam.questions?.map((q) => ({
                ...
                correctAnswer: typeof q.correctAnswer === 'object'
                    ? q.options?.findIndex((opt) => opt.isCorrect === true) ?? -1
...
router.get('/results', adminAuth_1.adminAuth, async (req, res) => {
    try {
        ...
        if (ctx && ctx.role !== 'super') {
            const myExams = await Exam_1.Exam.find({ createdBy: ctx.email }).select('_id').lean();
        ...
        const results = await ExamResult_1.ExamResult.find(filter)
...
// ✅ 3. جلب تفاصيل نتيجة معينة للمراجعة
router.get('/results/:resultId', adminAuth_1.adminAuth, async (req, res) => {
    try {
        const { resultId } = req.params;
        const result = await ExamResult_1.ExamResult.findById(resultId).lean();
        if (!result) {
        ...
        let exam = null;
        if (result.examId) {
            exam = await Exam_1.Exam.findById(result.examId).lean();
        }
        else {
            exam = await Exam_1.Exam.findOne({
                $or: [
...
router.patch('/results/:resultId', adminAuth_1.adminAuth, async (req, res) => {
    try {
        const { resultId } = req.params;
        ...
        if (Array.isArray(perQuestionOverrides))
            update.perQuestionOverrides = perQuestionOverrides;
        // ✅ إعادة حساب الدرجة
        const existing = await ExamResult_1.ExamResult.findById(resultId).lean();
        if (!existing) {
            return res.status(404).json({ success: false, error: 'النتيجة غير موجودة' });
        }
        const overridesMap = new Map();
        (perQuestionOverrides || existing.perQuestionOverrides || []).forEach((o) => {
        ...
        const updated = await ExamResult_1.ExamResult.findByIdAndUpdate(resultId, { $set: update }, { new: true, runValidators: true }).lean();
...
// ✅ حذف جماعي للنتائج
router.post('/results/bulk-delete', adminAuth_1.adminAuth, async (req, res) => {
    try {
        const { ids } = req.body;
        if (!Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({ success: false, error: 'No IDs provided' });
        }
        await ExamResult_1.ExamResult.deleteMany({ _id: { $in: ids } });
...
router.get('/results/export-pdf', adminAuth_1.adminAuth, async (req, res) => {
    try {
...
router.get('/results/export', adminAuth_1.adminAuth, async (req, res) => {
    try {
        const results = await ExamResult_1.ExamResult.find().sort({ submittedAt: -1 }).lean();
